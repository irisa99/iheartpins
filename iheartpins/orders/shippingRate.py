...
from .constant import LABEL_ZPL, SERVICE_PRIORITY
from iheartpins.settings import USPS_API_KEY
...
class USPSApi(object):
    ...
    def get_Rate(self, *args, **kwargs):
        return RateCalculator(self, *args, **kwargs)
...
class RateCalculator(object):
    ...
    def __init__(self, usps, zipOrigination, ZipDestination, weight, ounces,
                 service=SERVICE_PRIORITY, label_type=LABEL_ZPL):
        root = 'RateV4Request'
        xml = etree.Element(root, {'USERID': usps.api_user_id})

        label_params = etree.SubElement(xml, 'Revision')
        label_params.text = str(2)
        label = etree.SubElement(xml, 'Package', ID="0")

        delivery_service = etree.SubElement(label, 'Service')
        delivery_service.text = service

        zipOrigin = etree.SubElement(label, "ZipOrigination")
        zipOrigin.text = zipOrigination

        zipDest = etree.SubElement(label, "ZipDestination")
        zipDest.text = ZipDestination

        package_weight = etree.SubElement(label, 'Pounds')
        package_weight.text = str(weight)
        
        package_weight_ounce = etree.SubElement(label, 'Ounces')
        package_weight_ounce.text = str(ounces)

        c = etree.SubElement(label, 'Container')
        c.text = ""
       
        w = etree.SubElement(label, 'Width')
        w.text = ""

        l = etree.SubElement(label, 'Length')
        l.text = ""

        h = etree.SubElement(label, 'Height')
        h.text = ""
        
        g = etree.SubElement(label, 'Girth')
        g.text = ""
        
        machine = etree.SubElement(label, 'Machinable')
        machine.text = "TRUE"

        self.result = usps.send_request('rate', xml)


def getShippingRate(zipOrigination, zipDestination, weight_pound, weight_ounce):

    #Authenticate
    usps = USPSApi(USPS_API_KEY, test=True)
    label = usps.get_Rate(zipOrigination, zipDestination, weight_pound, weight_ounce, SERVICE_PRIORITY, LABEL_ZPL)
    json = label.result
    charge = json['RateV4Response']['Package']['Postage']['Rate']
    return charge 
